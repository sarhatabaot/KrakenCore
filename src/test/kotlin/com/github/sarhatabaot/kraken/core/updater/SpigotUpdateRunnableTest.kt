...
class SpigotUpdateRunnableTest {
    ...
    fun getApiEndpoint() {
        try {
            assertEquals(
                URL("https://api.spiget.org/v2/resources/%s/versions/latest".format("63972")),
    ...
    fun getNewVersionUrl() {
        assertEquals(
            "https://api.spiget.org/v2/resources/%s/versions/latest/download".format("63972"),
            outOfDateRunnable.getNewVersionUrl()
        )
        assertNotEquals(
            "https://api.spiget.org/v2/resources/%s/versions/latest/download".format("26543"),
            outOfDateRunnable.getNewVersionUrl()
        )
        assertNotEquals("https://incorrect.url.org/%s".format("63972"), outOfDateRunnable.getNewVersionUrl())
    }

    companion object {
        private lateinit var outOfDateRunnable : SpigotUpdateRunnable
        private lateinit var upToDateRunnable: SpigotUpdateRunnable
        @JvmStatic
        @BeforeAll
        fun setup() {
